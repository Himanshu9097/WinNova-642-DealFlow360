...
export default function EditQuotation() {
  ...
  useEffect(() => {
    if (params.id) {
      getQuotation(params.id)
        .then(data => {
          if (data) {
            setFormatType(data.formatType || 'Commercial');
  ...
  const totals = useMemo(() => {
    ...
    const margin = net * 0.15; // Simulated 15% margin
  ...
  const intelligenceData = {
    ...
    allowedDiscount: 8,
    marginPct: 15,
    riskScore: (discountPct > 8 ? 75 : 20) + (hasFulfillmentRisk ? 25 : 0),
  ...
  return (
    <div className="container">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          <h2 style={{color: '#D6536D'}}>Edit Quotation</h2>
          <span className="text-muted">Update your commercial offer.</span>
        </div>
        <div>
                        ...
                        <select 
                          ...
                          onChange={e => {
                            ...
                            if (prod) {
                              ...
                              newLines[index].maxDiscount = prod.maxDiscount !== undefined ? prod.maxDiscount : 100;
                              setLines(newLines);
                            } else {
